...
an exact form and the comptuing of analytical derivatives is shown. This prior
...
A work by W. Squire and G. Trapp showns through empirical data that CS offers superior accuracy
...
for use in Newton-Kyrlov schemes for solving non-linear systems. The instant
...
compute Jacobians for both integrating plastic stress to determine
elastoplastic modulii, but also for computing the Jacobian of the global
...
used in computing the global Jacobian  and said to be 'algorithmically
...
simulations on two plastic material models with complex hardening rules.
...
computer, CPU time data, and comparison to automatic differentiation as a competing method. 

Another work by A. Perez-Foguet, A. Rodriguez-Ferran, A. Huerta The prior work
applies CS in computing derivatives needed for integrating stress, obtaining
algorithmically consistent elastoplastic modulii, and obtaining the Jacobian of
the global residual, for FEM simulations using the the MRS-Lade material model
...
comparison to automatic differentiation as a competing method. 
...
general purpose programming languages. Also dicussed is the relative ease of
...
the using the CS method to obtain sensitivities.

A work by W. K. Anderson, J. C. Newman, D. l. Whitfield and E. J. Nielsen discusses
the CS applied to computing sensitivities in CFD simulations \cite{anderson2001sensitivity}. CS is
compared to AD both in Fortran and C++ applications. In this prior work it is suggested that forward mode
AD is mathematically equivalent to first order CS. Forware mode AD isThe instant work is novel with respect to the prior
work because the instant work investigates CS as applied to a numerical method
for solving non-linear systems, rather than as in the prior work investigating
...
comprising an analysis code. The analysis code can comprise a simulation and
derivatives of response variables called sensitivities can be computed. The
accuracy of this approach is compared to automatic differentiation
\cite{martins2003complex}. The instant work is novel with respect to the prior
work because the instant work investigates CS as applied to a numerical method
for solving non-linear systems, rather than as in the prior work investigating
the derivatives themselves produced with the CS method.
 
A work by  K. L. Lai and J. L. Crassidis discusses first and second order CS
methods for computing derivatives. Choosing optimal step size
\cite{lai2008extensions}. This prior work discusses computing first and second
order accurate derivatives with CS and choosing optimal step size.  The intant
work discusses using the derviatives computed with the CS method in a numerical
method for solving a non-linear system.

A work by A. H. Al-Mohy and N. J. Higham investigates the use of complex step
in computing Frechet derivatives of matix to matrix functions \cite{al2010complex}. To
compute the residual Jacobian in the peridynamics simulation discussed in the
instant work is analogous because it is computing a Frechet derivative of a
higher dimension function of higher dimension variables. Where the application
of the CS derivatives in the prior work is limited to a particular class of
abstract functions to determine the CS' suitability for use in condition number
estimation, the instant work applies CS to solving a mechanics problem. The
instant work is not about computing Frechet derivatives with CS but moreover using them
in a numerical method to solve a set of equations describing a physical system.
...
prior work the derivitives produced by CS themselves are the object of
interest, where the instant work uses CS in a numerical method for solving a
non-linear system.

A work by A. Voorhees, H. Millwater, and R. Bagley, where H. Millwater is an author of
the instant work, dicusses the background of the CS method and frames it as a subset
of Fourier Differentiation. CS and Fourier Differentiation are then applied to 
computing sensitivities within a solid mechanics FEA \cite{voorhees2011complex}. A notable
technique demonstrated by this prior work is the representation of complex valued
nodal components as additional degrees of freedom in the FE model, to be interpreted
by a matrix representation of multicomplex numbers during computations, yet without storing
the entire matrix. In the prior work, CS is discussed and related to a broader class of differentiation methods and used
to compute sensitivities. In the instant work, CS is used in a numerical method for solving non-linear systems.
...
elements is labelled ZFEM \cite{millwater2013application}. Notable in this work
is the discussion of creating user defined element types for use with the
popular \emph{ABAQUS} software suite which enable the use of ZFEM in that
...
with a re-interpreted version of the CS method which features the combined use
...
With respect to the prior work dicussing the CS method, the primary novel
aspects of the instant work are then its dicussion of the application of CS to
...
stored data elements are said to be 'byte-copiable' when the the interval
...
memory with a minimum of unrelated infomation stored in between data
...
ammount of state data according to their design and work done in the
