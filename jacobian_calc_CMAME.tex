...
\begin{figure}[h]
  \centering
  \scalebox{.7}{\input{./figs/serial_speed.pgf}}
...
and then forward-difference, taking the title of fastests method for this group of tests.
...
\begin{figure}[h]
  \centering
  \scalebox{.7}{\input{./figs/multi_speed.pgf}}
  \caption{Multicore test series speed measurements.}
  \label{fig:multi_speed}
\end{figure}

\subsection{Accuracy Data}
Accuracy measurements where taken according to the methods mentioned in \red{tcs}. The averaged
...
\begin{figure}[h]
  \centering
  \scalebox{.7}{\input{./figs/serial_accuracy.pgf}}
...
\begin{figure}[h]
  \centering
  \scalebox{.7}{\input{./figs/multi_accuracy.pgf}}
...
\begin{figure}[h]
  \centering
  \scalebox{.7}{\input{./figs/serial_speed_rel.pgf}}
...
\begin{figure}[h]
  \centering
  \scalebox{.7}{\input{./figs/multi_speed_rel.pgf}}
...
tests done in this study may give some users pause. While the results showed that complex-step was
clearly more accurate than central-difference and forward-difference for the parameters of the
tests, it was not determined whether or not this is a clear advantage of complex-step over
finite-difference in terms of accuracy of final results and speed of convergence.   For other
...
studies.  For reasons explained in the next subsection, a convergence study in \emph{Peridigm} performed using
the implementations of the methods used for this paper would be moot.

\subsection{Thoughts on \emph{Perdigm}}

After the bulk of the study had been conducted, gained familiarity with \emph{Peridigm} revealed
that the implementations of forward-difference, central-difference and by virtue of reusing code,
complex-step, in \emph{Peridigm} were not optimal due to inefficiencies in the implementations of
the Linear Peridynamic Solid (LPS) model. Each of these implementations included extensive looping
over noncontiguous addresses in memory and unnecessary re-calculation of constant values.  The
approach used does minimize temporary memory use, and preserve modularity of the code for non-LPS
models, but is very wasteful of cpu and memory bandwidth for use with LPS. Unpublished preliminary
results show that when calculations for the quantity 'zeta' are re-used and only perturbed and
previously pertubed functions of independent variables are recomputed throughout the course of a
Jacobian evaluation, complex-step performs at the speed of unmodified forward-difference. Because of
this it is speculated that forward-difference and central-difference could be modified in nearly the
same way to perform more quickly than AD. In fact, because forward difference involves double the
number of model evaluations than that of complex-step, forward-difference stands to gain
proportionally more efficiency. Although no formal academic paper will be likely written based on
this, a brief report and an enhanced performance implementation of forward-difference for peridynamic LPS
in \emph{Peridigm} will eventually be made available on the corresponding author's website and is
intended to be submitted for official inclusion in future releases of \emph{Peridigm}. 
...
compatible with the accelerator or GPU being used. What this means is that if \emph{Peridigm} or any
simulation software were to be reformulated to take advantage of accelerators or GPUs, attractive
given their performance and power efficiency advantages over traditional CPUs, the flexibilty of
complex-step owed to its simple implementation may make it more viable fo rthat application.
