...
underexplored method for numerical computation of tangent-stiffness operators,
...
Additionally, motivated by the reproducible research movement, all of the
...
determined by comparing its measured accuracy\note[Michael]{re:
I.1}\remove[Michael]{new} relative to the measured accuracy of popular finite
...
\emph{automatic differentiation} serve as the standard. \note[Michael]{re:
II.1}For the reader's convenience, from plasticity theory, the algorithmic
...
discussion. \note[Michael]{re: II.5}This open source resource is available at the corresponding author's website or at \href{https://github.com/utsa-idl/Complex_Step_Jacobian_Project_Archive}{https://github.com/utsa-idl/Complex\_Step\_Jacobian\_Project\_Archive}.
...
\note[Michael]{re: I.4, John all the rest of this section is new}
...
in computing Fr\'{e}chet derivatives of matrix to matrix functions
...
computing a Fr\'{e}chet derivative. Where the application of the CS derivatives in
...
about computing Fr\'{e}chet derivatives with CS but moreover using them in a
...
\note[Michael]{re: II.16, specific test parameters were asked for}
...
\note[Michael]{re: I.3}\add[Michael]{For the purposes of this work message passing is what
...
\add[Michael]{To summarize, a spacing of $1.E-6$ times grid spacing is used to provide acceptable accuracy but prevent roundoff error, 
...
%
\note[Michael]{re: II.22, relative accuracy figures were asked for, these were taken from the archive data}
...
\note[Michael]{re: I.5, this and other material was added in from the thesis to discuss accuracy}
\note[Michael]{re: I.5, the entire conclusion section was replaced with the thesis version}
\section{Conclusions}
\label{sec:Conc}
%
...
with the accelerator or GPU being used. \add[Michael]{A working definition of byte-copyable
...
still be an attractive method for TS matrix evaluation. \add[Michael]{However,
